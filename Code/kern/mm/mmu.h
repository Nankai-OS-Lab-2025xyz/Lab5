...
// PTE_COW: Copy-on-Write 标记位
// 用途：标识一个页面是 COW 共享页面（父子进程共享，但标记为只读）
// 机制：当进程尝试写入 COW 页面时，会触发页面错误，然后分配新页面并复制内容
// 位置：使用 PTE_SOFT 保留位中的 0x100（bit 8）
#define PTE_COW 0x100  // Software-defined: copy-on-write marker
...
// PF_WRITE: 页面错误是由写操作引起的
// 用途：在 do_pgfault 中判断是读错误还是写错误，COW 机制只处理写错误
#define PF_WRITE 0x2
